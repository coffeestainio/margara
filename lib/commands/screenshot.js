...
const taskStartBrowser = require('../tasks/start-playwright');
const taskStopBrowser = require('../tasks/stop-playwright');
...
async function screenshot(options) {
  ...
  for (const browserType of browsers) {
    ...
    tasksBrowser.push(taskStartBrowser(browserType));
    tasksBrowser.push(taskCollectScreenshots(captureSettings));
    tasksBrowser.push(taskStopBrowser(browserType));
  ...
  const tasks = new Listr([
    {
      ...
      task: () => {
        return new Listr(tasksBrowser, {concurrent: false});
      }
    },
  ]);
